...
==File Format==
The files are plain text files, where rows and columns are separated by the token character [[wikipedia:Hash_symbol|#]]. It can be edited with every text editor like Notepad. Make sure, that the file is not stored in [[wikipedia:Unicode|Unicode]] or [[wikipedia:UTF-8|UTF-8]].
...
<pre>/savechat : saves chat as text file.
#
